...
an array of one-letter strings representing directions to walk (eg. ['n', 's', 'w', 'e']). You always walk only 
...
function isValidWalk(walk) {
  if (Array.isArray(walk) && walk.length === 10){
    return walk.filter(x => x === 'n').length === walk.filter(x => x === 's').length &&
